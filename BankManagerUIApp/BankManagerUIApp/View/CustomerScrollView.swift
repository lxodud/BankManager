...
class CustomerQueueScrollView: UIScrollView {
    ...
    func addCustomerLabel(customer: Customer) {
        let customerLabel: CustomerLabelStackView = {
            let stackView = CustomerLabelStackView()
            stackView.addCustomerLabelText(customer: customer)
            return stackView
        }()
